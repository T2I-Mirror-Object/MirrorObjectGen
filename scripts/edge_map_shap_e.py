...
def generate_edge_for_prompt(
    ...
    """
    Generate an edge map for a given text prompt.
    ...
    Returns:
        str: Path to the generated edge map image
    """
    ...
    print("=" * 60)
    print("PyTorch3D Scene Composition & Edge Extraction")
    print("=" * 60)
    ...
    print(f"  ✓ Edge map saved to: {edge_map.image_path}")
    print("\nEdge extraction completed successfully!")
    
    return edge_map.image_path


if __name__ == "__main__":
    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description='Generate 3D scene with mirror reflections and edge map from text prompt'
    )
    parser.add_argument(
        '--prompt',
        type=str,
    ...
    edge_map_path = generate_edge_for_prompt(
        prompt=args.prompt,
    ...
    print(f"\n{'=' * 60}")
    print(f"Final output: {edge_map_path}")
    print(f"{'=' * 60}")
